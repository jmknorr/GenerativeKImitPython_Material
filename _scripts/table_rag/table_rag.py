...
sql_query['sql_query']
# %%
fetch_information_from_db(query="""
        SELECT region, COUNT(*) as light_roast_count FROM coffee_sales WHERE roast_level = 'Light' GROUP BY region ORDER BY light_roast_count DESC;
""")
...
def rag(user_query: str, sql_table_info: str):
    ...
    # prepare chain
    messages = [
        ("system", "Du bist Experte für Kaffee und kannst Fragen zu den Daten in der Tabelle beantworten. Bitte antworte nur auf Basis der Liste der relevanten Informationen. Diese Antwort kam von der Datenbank: {retrieved_information}. Die zugrundeliegende SQL-Query war: {sql_query}. Bitte antworte in deutscher Sprache."),
        ("user", "{user_query}")
    ]
